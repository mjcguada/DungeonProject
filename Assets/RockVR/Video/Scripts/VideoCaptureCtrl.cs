            ...
            set
            {
                if (status == StatusType.STARTED)
                {
                    Debug.LogWarning("[VideoCaptureCtrl::AudioCapture] Cannot " +
                                     " set aduio during capture session!", DLogType.Warning);
                    return;
                }
                _audioCapture = value;
            }
        ...
        public override void StartCapture()
        {
                ...
                status != StatusType.FINISH)
            {
                Debug.LogWarning("[VideoCaptureCtrl::StartCapture] Previous " +
                                 " capture not finish yet!", DLogType.Warning);
                return;
            }
            ...
            if (!isCaptureAudio && videoCaptures.Length == 0)
            {
                Debug.LogError(
                    ...
                    "but no attached VideoRecorder or AudioRecorder were found!", DLogType.Error
                );
                return;
            }
            if (!File.Exists(PathConfig.ffmpegPath))
            {
                Debug.LogError(
                    ...
                    "ffmpeg executable before start capture!", DLogType.Error
                );
                return;
            }
